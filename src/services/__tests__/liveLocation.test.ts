...
const mockDeleteDoc = jest.fn(async (..._args: unknown[]) => undefined);
...
describe('startSharingLocation', () => {
  it('encrypts the position for the peer and writes expiresAt/updatedAt', async () => {
    ...
    // Round-trips through real decryption — proves this isn't just structurally
    // shaped like an EncryptedPayload but actually decryptable by the peer.
