 ...
 * animated backdrop (see ThemeBackdrop.tsx) that replaces the old flat
 * wallpaper tint; `wallpaper` itself stays as the static reduced-motion
 * fallback rather than becoming a dead field.
 */
...
export function isDarkWallpaper(wallpaper: string | null | undefined): boolean {
  const brightness = perceivedBrightness(wallpaper);
