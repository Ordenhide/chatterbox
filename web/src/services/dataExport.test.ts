...
function clauseMatches(data: Record<string, unknown>, clause: {field: string; op: string; value: unknown}): boolean {
...
vi.mock('firebase/firestore', () => ({
  ...
  getDocs: async (ref: any) => {
    ...
    const filtered = all.filter(d => (ref.clauses || []).every((c: any) => clauseMatches(d.data, c)));
