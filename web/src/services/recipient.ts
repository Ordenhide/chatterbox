 ...
 *
 * Mirrored by src/services/recipient.ts on mobile — keep the two in step.
 */
 ...
 * one. Reads from the server rather than the local cache: a peer whose profile
 * was cached before they deleted their account is exactly the case this is for.
 */
 ...
 * single message, and the open chat pane already holds a live listener on this
 * exact document, so `getDoc` is served from the local cache without a network
 * round trip. The profile check is deliberately left to the UI, which runs it
 * once per chat open and disables the composer — paying for a second server
 * read on every keystroke-to-send would not buy anything the banner hasn't
 * already prevented.
 *
