 ...
 */
import {collection, doc, getDoc, getDocs, query, where, type Query} from 'firebase/firestore';
 ...
 * ChatPane.tsx's own decrypt loop (placeholder text there, a status flag
...
async function dumpDocs(q: Query): Promise<Record<string, unknown>[]> {
  const snap = await getDocs(q);
  return snap.docs.map(d => ({id: d.id, ...(sanitizeForExport(d.data()) as object)}));
}

/** Runs several queries and merges their results, de-duplicated by doc id. */
async function dumpDocsUnion(queries: Query[]): Promise<Record<string, unknown>[]> {
...
async function fetchChatExport(
  ...
  const messages: Record<string, unknown>[] = messagesSnap.docs.map(d => {
