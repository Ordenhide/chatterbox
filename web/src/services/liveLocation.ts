 ...
 *
 * Foreground-only by design: nothing here keeps running once the component
 * that started the watch unmounts — see the web UI's use of
 * `watchMyPosition` (src/utils/geolocation.ts), which ChatPane tears down
 * on unmount/chat switch.
 */
...
/**
 * Starts sharing this browser's location with `peerUid` in `chatId` for
