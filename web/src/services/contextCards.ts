/**
 * Wikipedia "context cards" for entities mentioned in a message.
 *
 * A verbatim port of the mobile service — it is pure logic plus fetch(), with
 * no Firebase and no platform APIs, so the two stay identical by construction.
 * Keep them in step: the entity extraction and stop-word list are the whole
 * feature, and a divergence would show as different cards on each client for
 * the same message.
 */
