/**
 * Cryptographic primitives for Chatterbox — web client.
 *
 * This is a verbatim port of the mobile app's src/services/crypto.ts. Both are
 * built on @noble/* (pure-JS, audited, no native code), which is exactly what
 * lets them share one implementation: browsers provide `crypto.getRandomValues`
 * natively (no polyfill needed here, unlike Hermes on React Native), so nothing
 * else differs. Keep this file byte-for-byte in sync with the mobile original —
 * any divergence risks mobile and web deriving different keys from the same
 * secret and silently failing to decrypt each other's messages.
 *
 * Deliberate choices:
...
/** Random 32-byte key, hex-encoded. */
